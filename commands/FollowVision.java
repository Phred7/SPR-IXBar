 ...
 */
	...
	public class FollowVision extends Command {
		...
		// Angle from center on y-axis
		double yOffset;
		// Angle from center on y-axis (corrected sign)
		double headingError;
		...
		protected void execute() {
			// Get current offset on the y-axis
			yOffset = Robot.limelight.getYOffset();
			// Current heading error
			headingError = -yOffset;
					SmartDashboard.putNumber("FollowVision().headingError", headingError);
			...
			if (headingError > errorMargin) { // If the offset is greater than one, subtract the minimum
				...
				isFinished = true;
					...
					SmartDashboard.putNumber("FollowVision().leftSpeed", leftSpeed);
					SmartDashboard.putNumber("FollowVision().rightSpeed", rightSpeed);
		...
		@Override
		protected void end() {
			System.out.println("FollowVision(): Target aligned!");
		}
